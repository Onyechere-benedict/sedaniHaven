document.addEventListener("DOMContentLoaded", function () {
	...
	if (bookButtons.length > 0 && modal) {
		bookButtons.forEach((button) => {
			button.addEventListener("click", function () {
				...
				selectedRoomPrice = parseInt(roomType);
	...
	if (payWithCard) {
		payWithCard.addEventListener("click", function () {
			...
			let paystackHandler = PaystackPop.setup({
				key: "pk_test_fc5250f76725adf7786f405f652f3da2bc3e3890",
				email: "customer@example.com",
				amount: selectedRoomPrice * 100,
				currency: "NGN",
	...
	function animateOnScroll() {
		const animatedElements = document.querySelectorAll(
			".feature-card, .room-card, .gallery-item, .about-image, .about-text, .mission-text, .mission-image, .value-card, .team-member"
		);
